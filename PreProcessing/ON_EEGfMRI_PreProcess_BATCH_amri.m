...
addpath(genpath('/Users/alexgonzalez/Documents/MATLAB/Mylib/Filters'))
...
for sub = subj;
    ...
    for r = 1:5%runs
        ...
        S.signal = FIR_Filter(S.signal,S.fs,S.lowpasscut,S.highpasscut);
        eeg = [];
        eeg.data = S.signal;
        eeg.srate = S.fs;
        eeg.chanlocs = readlocs('GSN-HydroCel-257.sfp','filetype','sfp');
        
        for ev = 1:numel(S.MR_Pulses_orig)
            eeg.event(ev).latency = S.MR_Pulses_orig(ev);
            eeg.event(ev).type    = 'MRpulse';
        end
