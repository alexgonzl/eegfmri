...
case 'obs'
    ...
    %fs=EEG.srate; 
    fs=EEG.SR; 
    [channels samples]=size(EEG.signal);
    pcs=npc-1;
...
%         end
        ...
        eegchan=filtfilt(fwts,1,double(EEG.signal(ch,:)));
       ...
       try
           pad_fit=double(papc)*(double(papc)\...
                double(detrend(EEG.signal(ch,peakI(1)-PArange:...
        ...
        for p=2:GHW+1
           ...
           pad_fit=double(papc)*(double(papc)\...
                double(detrend(EEG.signal(ch,peakI(p)-PArange:...
    ...
    EEG.signal=EEG.signal-fitted_art;
...
otherwise
    ...
    for n=1:channels
        pdrift=polyfit(t,EEG.signal(n,1:peakI(GHW+1)+PArange),1);
        drift(n,1:peakI(GHW+1)+PArange)=polyval(pdrift,t);
    end

    EEG.signal(:,1:peakI(GHW+1)+PArange)=...
        EEG.signal(:,1:peakI(GHW+1)+PArange)-...
    ...
    for p=2:GHW+1
        avgdata(:,1:2*PArange+1,p-1)=...
            EEG.signal(:,peakI(p)-PArange:peakI(p)+PArange);
    end
    ...
    if peakI(1) > PArange
        ...
        alphaV=...
            sum(detrend(EEG.signal(:,peakI(1)-PArange:peakI(1)+PArange)','constant')'...
            .*mPA(:,1:2*PArange+1),2)./...
        ...
        alphaV=sum(detrend(EEG.signal(:,1:peakI(1)+PArange)','constant')'...
    ...
    if peakI(1) > PArange
        ...
        clean(:,peakI(1)-PArange:peakI(1)+PArange)=...
            EEG.signal(:,peakI(1)-PArange:peakI(1)+PArange)-alphaD*mPA(:,1:2*PArange+1);
        startpoints=peakI(1)-PArange-1;
        clean(:,1:startpoints)=EEG.signal(:,1:startpoints);
    else

        clean(:,1:peakI(1)+PArange)=...
            EEG.signal(:,1:peakI(1)+PArange)-alphaD*mPA(:,PArange-peakI(1)+2:2*PArange+1);
    end
    ...
    for p=2:GHW+1
        ...
        alphaV=sum(EEG.signal(:,peakI(p)-PArange:peakI(p)+PArange).*...
            mPA(:,1:2*PArange+1),2)./...
            sum(mPA(:,1:2*PArange+1).*mPA(:,1:2*PArange+1),2);
    ...
    for p=GHW+2:peakcount-GHW-1 
        ...
        for n=1:channels
            pdrift=...
                polyfit(t,EEG.signal(n,peakI(p-GHW)-PreP:peakI(p+GHW)+PostP),1);
            drift(n,1:sectionPoints)=polyval(pdrift,t);
        end

        EEG.signal(:,peakI(p-GHW)-PreP:peakI(p+GHW)+PostP)=...
            EEG.signal(:,peakI(p-GHW)-PreP:peakI(p+GHW)+PostP)-...
        ...
        for r=p-GHW:p+GHW
            mMag(:,rr)=mean(abs(EEG.signal(:,peakI(r)-PreP:peakI(r)+PostP)),2);
            rr=rr+1;
        end
        ...
        rcount=0;
        for r=p-GHW:p+GHW
            if (mean(abs(EEG.signal(1,peakI(r)-PreP:peakI(r)+PostP))) ...
                    < 4*minMag(1) ) ...
                    & (max(abs(EEG.signal(1,peakI(r)-PreP:peakI(r)+PostP)))...
                    < 2* 175)        
                avgdata(:,1:PreP+PostP+1,rcount+1)=...
                    EEG.signal(:,peakI(r)-PreP:peakI(r)+PostP);
                rcount=rcount+1;
            end
        end
    ...
    %     rcount
        ...
        alphaV=sum(detrend(EEG.signal(:,peakI(p)-PreP:peakI(p)+PostP)','constant')'.*...
        ...
        clean(:,peakI(p)-PreP:peakI(p)+PostP)=...
            EEG.signal(:,peakI(p)-PreP:peakI(p)+PostP)-...
    ...
    if samples-peakI(peakcount) >= PostP
        ...
        alphaV=sum(detrend(EEG.signal(:,peakI(peakcount)-...
        ...
        clean(:,peakI(peakcount)-PreP:peakI(peakcount)+PostP)=...
            EEG.signal(:,peakI(peakcount)-PreP:peakI(peakcount)+PostP)- ...
                            alphaD*mPA(:,1:PreP+PostP+1);

        endpoints=samples-peakI(peakcount)-PostP;

        clean(:,samples-endpoints+1:samples)=...
            EEG.signal(:,samples-endpoints+1:samples);        

    else

        alphaV=sum(detrend(EEG.signal(:,peakI(peakcount)-PreP:samples)','constant')'.*...
        ...
        clean(:,peakI(peakcount)-PreP:samples)=...
            EEG.signal(:,peakI(peakcount)-PreP:samples)-...
