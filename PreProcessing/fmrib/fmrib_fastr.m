...
if d2 > length(EEG.signal)
    d2=length(EEG.signal);
end
...
if sections>1
    while SecTFlag==0
        if (d1-1)+floor(SecT*fs)+pad_sec > EEG.SampsPerSlice
            sections=1;
            SecTENDFlag=1;
            break;
        end
...
for sec=1:sections
    ...
    if L > 1
        if sec==1 & sections > 1
            Idata=interp(EEG.signal(c,d1:(d1-1)+secl(sec)+pad_sec),L,4,1);
        elseif sec==1 & sections==1
            Idata=interp(EEG.signal(c,d1:d2),L,4,1);
        elseif sec==sections
            Idata=interp(EEG.signal(c,(d1-1)+sum(secl(1:sec-1))+1-pad_sec:...
                d2),L,4,1);
        else
            Idata=interp(EEG.signal(c,(d1-1)+sum(secl(1:sec-1))+1-pad_sec:...
        ...
        if sec==1 & sections > 1
            Idata=EEG.signal(c,d1:(d1-1)+secl(sec)+pad_sec);
        elseif sec==1 & sections==1
            Idata=EEG.signal(c,d1:d2);
        elseif sec==sections
            Idata=EEG.signal(c,(d1-1)+sum(secl(1:sec-1))+1-pad_sec:d2);
        else
            Idata=EEG.signal(c,(d1-1)+sum(secl(1:sec-1))+1-pad_sec:...
...
for c=1:m
    ...
    tmpdata=EEG.signal(c,:)-mean(EEG.signal(c,:));
    
    cleanEEG=EEG.signal(c,:);
    ...
    for sec=1:sections
        ...
        if L > 1
            if sec==1 & sections > 1
                ...
                Iorig=interp(EEG.signal(c,d1:(d1-1)+secl(sec)+pad_sec),L,4,1);
            elseif sec==1 & sections==1
                Idata=interp(tmpdata(d1:d2),L,4,1);
                Iorig=interp(EEG.signal(c,d1:d2),L,4,1);
            elseif sec==sections
                Idata=interp(tmpdata((d1-1)+...
                    sum(secl(1:sec-1))+1-pad_sec:d2),L,4,1);
                Iorig=interp(EEG.signal(c,(d1-1)+...
                    sum(secl(1:sec-1))+1-pad_sec:d2),L,4,1);
            else
                Idata=interp(tmpdata((d1-1)+sum(secl(1:sec-1))+...
                    1-pad_sec:(d1-1)+sum(secl(1:sec))+pad_sec),L,4,1);
                Iorig=interp(EEG.signal(c,(d1-1)+sum(secl(1:sec-1))+...
                    1-pad_sec:(d1-1)+sum(secl(1:sec))+pad_sec),L,4,1);
            end
        else
            if sec==1 & sections > 1
                Idata=tmpdata(d1:(d1-1)+secl(sec)+pad_sec);
                Iorig=EEG.signal(c,d1:(d1-1)+secl(sec)+pad_sec);
            elseif sec==1 & sections==1
                Idata=tmpdata(d1:d2);
                Iorig=EEG.signal(c,d1:d2);
            elseif sec==sections
                Idata=tmpdata((d1-1)+sum(secl(1:sec-1))+1-pad_sec:d2);
                Iorig=EEG.signal(c,(d1-1)+sum(secl(1:sec-1))+1-pad_sec:d2);
            else
                Idata=tmpdata((d1-1)+sum(secl(1:sec-1))+1-pad_sec:...
                    (d1-1)+sum(secl(1:sec))+pad_sec);
                Iorig=EEG.signal(c,(d1-1)+sum(secl(1:sec-1))+1-pad_sec:...
                    (d1-1)+sum(secl(1:sec))+pad_sec);
            end
        end
    ...
    if anc_chk==1
        ...
        if isinf(max(y))
            ...
            warning(wst);
            EEG.signal(c,d1:d2)=cleanEEG(d1:d2);
        else
            EEG.signal(c,d1:d2)=cleanEEG(d1:d2)-y';
        end
    else
        EEG.signal(c,d1:d2)=cleanEEG(d1:d2);
    end
