...
addpath('~/Documents/MATLAB/Mylib/Filters/')
% subject number
subj = [22:27];
% runs
runs = [1 2 3 4 5];
...
for sub = subj;
    ...
    ext = '.mat';
    %mainpath1 = ['/Volumes/EXT_HD/ON_eegfmri/s' num2str(sub)];
    mainpath1 = ['~/Documents/EEG_FMRI/RawData/s' num2str(sub)];
    
    datapath = [mainpath1 '/raw/'];
    ...
    for r = 1:5%runs
        ...
        S.filename = [datapath varname num2str(r) ext];
        
        % load data
        temp = load(S.filename);
        % signal
        S.signal = single(temp.([varname num2str(r) suffix]));
        % reference channel
        S.refchannel = 257;
        ...
        % filter signal
        S.GAC_filt_signal = FIR_Filter(S.GAClean_signal, ...
            S.fs,S.lowpasscut,S.highpasscut,S.notch);
        
        % clean the pulse artifcat
        S = ST_PA_Clean_v3(S);
        
        S.recons_signal = FIR_Filter(S.recons_signalpad, ...
            S.fs,S.lowpasscut,S.highpasscut,S.notch);
        ...
        %S.GAC_filt_signal = [];
        if ~exist([mainpath1 '/PreProccesedData/r' num2str(r)'],'dir')
            mkdir([mainpath1 '/PreProccesedData/r' num2str(r)'])
            %mkdir([mainpath2 '/clean_data2/r' num2str(r)'])
        end
        save([mainpath1 '/PreProccesedData/r' num2str(r) '/data'],'S','-v7.3')
